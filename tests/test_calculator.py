...
class TestCalculator:
    ...
    def test_clamp_invalid_range(self):
        with pytest.raises(
            ValueError, match="min_value must not be greater than max_value"
        ):
            clamp(5, 10, 0)
    ...
    def test_round_to(self):
        ...
        assert round_to(1.005, 2) == 1.01
        assert round_to(2.5, 0) == 3
        assert round_to(123.456) == 123

    def test_round_to_negative_decimals(self):
